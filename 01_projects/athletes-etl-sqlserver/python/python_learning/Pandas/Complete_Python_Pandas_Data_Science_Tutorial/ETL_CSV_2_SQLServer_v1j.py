# ADDED CHUNKED INSERTS (CHUNKSIZE=5000)

# What this does (simple)

# If IS_CHUNKED_INSERTS = True → inserts happen in batches of CHUNK_SIZE rows (e.g., 5000 rows at a time).

# If IS_CHUNKED_INSERTS = False → no chunksize is passed, 
# so Pandas does its default behavior (one big insert operation).
...
# ✅ Chunked inserts toggle
IS_CHUNKED_INSERTS = False      # True = use chunksize, False = normal insert
CHUNK_SIZE = 5000              # change to any size you want (e.g., 10000)
...
try:
    ...
    athletes.to_sql(
        ...
        if_exists="append",
        index=False,
        **to_sql_kwargs # ********************  ** unpacks the dictionary => **to_sql_kwargs = chunksize=5000
